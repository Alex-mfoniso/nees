...
app.put(
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (exchangeable !== undefined) updateData.exchangeable = exchangeable === "1" || exchangeable === true;
            if (refundable !== undefined)   updateData.refundable   = refundable === "1" || refundable === true;
...
app.get("/debug/product-count", async (req, res) => {
    try {
        ...
        res.json({ status: "OK", productCount: count, message: "Data from MongoDB Atlas." });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
